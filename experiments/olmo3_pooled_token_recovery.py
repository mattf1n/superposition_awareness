...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Test whether an OLMo 3 model can verbalize tokens from pooled embeddings."
    )
    parser.add_argument(
        "--model",
...
def build_prompt(
    ...
    if mode == "single":
        user_content = (
            ...
            "Recover that word.\n"
            "Reply with only the word and no explanation.\n\nTOKEN:\n"
            f"{placeholder_text}"
        )
    else:
        ...
        user_content = (
            ...
            "Reply with only the two words, separated by a comma, and no explanation.\n\nTOKEN:\n"
            f"{placeholder_text}"
        )
    ...
    if getattr(tokenizer, "chat_template", None):
        ...
        return torch.tensor([input_ids]), occurrences[0], prompt_suffix
    ...
    return torch.tensor([input_ids]), placeholder_index, prompt_suffix
...
def evaluate_trial(
    ...
    input_ids, placeholder_index, prompt_suffix = build_prompt(
        tokenizer,
...
def main() -> None:
    ...
    for token_id, token_word in single_targets:
        result = evaluate_trial(
            ...
            placeholder_text=placeholder_word,
    ...
    output_path = args.output_dir / f"olmo3_pooled_token_recovery_{timestamp}.json"
    payload = {
